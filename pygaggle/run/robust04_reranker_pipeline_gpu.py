...
queries and the retrieval MCRanker_run file for the queries.
...
def load_run(path):
    """Loads MCRanker_run into a dict of key: query_id, value: list of candidate doc
    ids."""
    print('Loading MCRanker_run...')
...
def parse_args(args):
    parser = argparse.ArgumentParser(
        description='Create T5-formatted input from Robust04 queries, MCRanker_run and corpus. After that, it uses a monoT5-base to rerank the candidates. The reranked output is TREC-formatted.')
    parser.add_argument('--queries', type=str, required=True,
                        help='Path to TREC-formatted queries (topics) file.')
    parser.add_argument('--MCRanker_run', type=str, required=True,
                        help='Path to TREC-formatted MCRanker_run file containing the candidates'
...
for query_id, doc_ids in tqdm(run.items(), total=len(run)):
    ...
    # Writes a MCRanker_run file in the TREC format
