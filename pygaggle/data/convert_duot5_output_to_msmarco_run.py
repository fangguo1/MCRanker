"""
This script convert duoT5 output file to msmarco MCRanker_run file
"""
import argparse
import collections
import numpy as np
...
parser.add_argument("--input_run", type=str, required=True,
                    help="path to input MCRanker_run, tsv file, with <query_id>, <doc_id> and <rank>")
parser.add_argument("--duo_run", type=str, required=True,
                    help="path to output duo MCRanker_run, tsv file, with <query_id>, <doc_id> and <rank>")
...
def load_run(path):
    ...
    # We want to preserve the order of runs so we can pair the MCRanker_run file with
    # the TFRecord file.
