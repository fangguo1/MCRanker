...
def get_identity_score(i, query, docid_doc_list, dataset_path, identity, criteria):
    qbs_prompt = prompt_builder('identity_score_no_reason_USE', load_from_warehouse=True,
                                warehouse_file='../prompt_warehouse.json')
    qbs_prompt.prefix = qbs_prompt.prefix.replace('#IDENTITY#', identity)

    # print(qbs_prompt.prefix)
    input_variables = ["query", "criteria", "passage"]
    all_prompts = []
    pred_infos = []

    print(f"{i} :{query}")
    for (idx, doc) in docid_doc_list:
        input_values = [query, criteria, doc]
        input_dict = {}
    ...
    make_qbs_prompts(pred_infos, all_prompts,
                     f'Multi_agent_{str(i)}_requests_{identity}_score_without_reason_dataset_criteria_gpt4_1106.json',
    ...
    result_file = os.path.join(os.path.join(dataset_path, 'output_bm25_100'),
                               f'Multi_agent_{str(i)}_results_{identity}_score_without_reason_dataset_criteria_gpt4_1106.json')
...
def get_linguist_score(i,query,docid_doc_list,dataset_path,criteria):
    qbs_prompt = prompt_builder('linguist_score_no_reason_USE', load_from_warehouse=True,
                                warehouse_file='../prompt_warehouse.json')
    input_variables = ["criteria","passage", "question"]
    all_prompts = []
    pred_infos = []
    ...
    for (idx, doc) in docid_doc_list:
        input_values = [criteria,doc, query]
        input_dict = {}
        for key, value in zip(input_variables, input_values):
            input_dict[key] = value
        qbs_prompt.example_maker(input_variables=input_variables, input_dict=input_dict)
    ...
    make_qbs_prompts(pred_infos, all_prompts, f'Multi_agent_{str(i)}_requests_linguist_score_without_reason_dataset_criteria_gpt4_1106.json',
                     dataset_path, iterations=1)
    request_file = os.path.join(os.path.join(dataset_path, 'requests_bm25_100'),
                                f'Multi_agent_{str(i)}_requests_linguist_score_without_reason_dataset_criteria_gpt4_1106.json')
    result_file = os.path.join(os.path.join(dataset_path, 'output_bm25_100'),
                               f'Multi_agent_{str(i)}_result_linguist_score_without_reason_dataset_criteria_gpt4_1106.json')
    run_multithread_gpt(request_file, result_file,api_key=API_KEY)
...
for dataset in datasets:
    ...
    query_file = f'query_{1}.txt'
    pasasge_file = f'passages_bm25_100_{1}.txt'
    ...
    lin_criteria, linguist_reason = linguist_criteria(dataset[2], 'gpt-4-1106-preview')

    identities_raw, reason = generate_identities(dataset[2], dg.docid2doc[2], identity_num, 'gpt-4-1106-preview')
    identities = []
    ...
    with open(f'{dataset_path}/output_bm25_100/Multi_agent_{dataset[0]}_identities_without_reason_dataset_criteria_gpt4_1106_domain_{str(identity_num)}.txt',
              'w+') as OUT:
    ...
    for identity in identities:
        criteria, reason = generate_criteria_per_identity(identity, dataset[2], model_name='gpt-4-1106-preview')
        identity_info = {'name': identity, 'criteria': criteria, 'reason': reason}
        identity_info_list.append(identity_info)

    with open(f'{dataset_path}/output_bm25_100/Multi_agent_{dataset[0]}_identities_{str(identity_num)}_without_reason_dataset_criteria_gpt4_1106.json','w+') as f:
        for identity_info in identity_info_list:
            json_string = json.dumps(identity_info)
            f.write(json_string + "\n")

    with open(f'{dataset_path}/output_bm25_100/Multi_agent_{dataset[0]}_identities_{str(identity_num)}_without_reason_dataset_criteria_gpt4_1106.json') as IN:
        identity_info_list=[]
        identities=[]
        for line in IN:
            line=line.strip()
    ...
    for i in dataset[1]:
        ...
        get_linguist_score(i,dg.query, docid_doc_list, dataset_path,str(lin_criteria))

        for tp in identity_info_list:
            identity=tp['name']
            criteria=tp['criteria']
            get_identity_score(i,dg.query,docid_doc_list,dataset_path,identity,criteria)
        ### End First Step ###
        ...
        for j in range(len(score_name)):
            result_file = os.path.join(os.path.join(dataset_path, 'output_bm25_100'),
                                       f'Multi_agent_{str(i)}_results_{score_name[j]}_score_without_reason_dataset_criteria_gpt4_1106.json')
