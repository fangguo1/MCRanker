...
def get_identity_score(i, query, docid_doc_list, dataset_path, identity, criteria):
    ...
    make_qbs_prompts(pred_infos, all_prompts,
                     f'Multi_agent_{str(i)}_requests_{identity}_score_without_reason_gpt4_1106.json',
    ...
    result_file = os.path.join(os.path.join(dataset_path, 'output_bm25_100'),
                               f'Multi_agent_{str(i)}_results_{identity}_score_without_reason_gpt4_1106.json')
...
def get_linguist_score(i,query,docid_doc_list,dataset_path,criteria):
    ...
    make_qbs_prompts(pred_infos, all_prompts, f'Multi_agent_{str(i)}_requests_linguist_score_without_reason_gpt4_1106.json',
                     dataset_path, iterations=1)
    request_file = os.path.join(os.path.join(dataset_path, 'requests_bm25_100'),
                                f'Multi_agent_{str(i)}_requests_linguist_score_without_reason_gpt4_1106.json')
    result_file = os.path.join(os.path.join(dataset_path, 'output_bm25_100'),
                               f'Multi_agent_{str(i)}_results_linguist_score_without_reason_gpt4_1106.json')
    run_multithread_gpt(request_file, result_file,api_key=API_KEY)
...
for dataset in datasets:
    ...
    for i in dataset[1]:
        ...
        lin_criteria,linguist_reason=linguist_criteria(dg.query,'gpt-4-1106-preview')
        ...
        with open(f'{dataset_path}/output_bm25_100/Multi_agent_{i}_identities_without_reason_{str(identity_num)}_gpt4_1106.txt','w+') as OUT:
        ...
        with open(f'{dataset_path}/output_bm25_100/Multi_agent_{i}_identities_{str(identity_num)}_without_reason_gpt4_1106.json','w+') as f:
