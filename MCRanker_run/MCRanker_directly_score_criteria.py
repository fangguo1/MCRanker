...
def get_gpt_score(i,query,docid_doc_list,dataset_path,criteria):
    qbs_prompt = prompt_builder('score_0to10_criteria_no_reason_USE', load_from_warehouse=True,
                                warehouse_file='../prompt_warehouse.json')
    input_variables = ["criteria", "question","passage"]
    all_prompts = []
    pred_infos = []
    ...
    for (idx, doc) in docid_doc_list:
        input_values = [criteria, query,doc]
        input_dict = {}
    ...
    make_qbs_prompts(pred_infos, all_prompts, f'Multi_agent_{str(i)}_requests_score_0to10_criteria_without_reason_gpt4_1106.json',
                     dataset_path, iterations=1)
    request_file = os.path.join(os.path.join(dataset_path, 'requests_bm25_100'),
                                f'Multi_agent_{str(i)}_requests_score_0to10_criteria_without_reason_gpt4_1106.json')
    result_file = os.path.join(os.path.join(dataset_path, 'output_bm25_100'),
                               f'Multi_agent_{str(i)}_results_score_0to10_criteria_without_reason_gpt4_1106.json')
...
for dataset in datasets:
    ...
    for i in dataset[1]:
        ...
        with open(f'{dataset_path}/output_bm25_100/Multi_agent_{i}_identities_0to10_criteria_without_reason_gpt4_1106_{str(identity_num)}.txt',
                  'w+') as OUT:
        ...
        with open(
                f'{dataset_path}/output_bm25_100/Multi_agent_{i}_identities_{str(identity_num)}_0to10_criteria_without_reason_gpt4_1106.json',
                'w+') as f:
        ...
        with open(
                f'{dataset_path}/output_bm25_100/Multi_agent_{i}_identities_{str(identity_num)}_0to10_criteria_without_reason_gpt4_1106.json') as IN:
            identity_info_list = []
            identities = []
            for line in IN:
                line = line.strip()
        ...
        criteria=''
        criteria+=f'\nCriteria from the view of linguist:\n<<<{lin_criteria}>>>\n'
        for tp in identity_info_list:
            identity = tp['name']
            criteria+=f"\nCriteria from the view of {identity}:\n<<<{tp['criteria']}>>>\n"


        get_gpt_score(i,dg.query,docid_doc_list,dataset_path,criteria)
        ...
        for j in range(30):
            ...
            final_scores.append((key, int(scores_list['gpt_0to10'][
                                              key])))
